...
par(mfrow = c(2, 2))
with(hpc, {
  plot(x, as.numeric(levels(Global_active_power))[Global_active_power], type = "l",
       xlab = "", ylab = "Global Active Power")
  plot(x, as.numeric(levels(Voltage))[Voltage], type = "l", xlab = "datetime",
       ylab = "Voltage")
  ...
  legend("topright", lty = 1, col = c("black", "blue", "red"), legend = names(hpc)[-1:-6],
         cex = 0.75, bty = "n", xjust = 1, y.intersp = 0.25)
  plot(x, as.numeric(levels(Global_reactive_power))[Global_reactive_power], type = "l",
